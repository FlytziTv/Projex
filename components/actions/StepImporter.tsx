    ...
    reader.onload = async (event) => {
      try {
        ...
        const response = await fetch(
          `http://localhost:3001/api/projects/${projectId}/import-steps`,
          {
            method: "POST",
        ...
        if (response.ok) {
          ...
          alert(
            "Erreur d'importation: " + (errorData.error || response.statusText),
          );
        }
      } catch (err) {
        ...
        // Important : reset l'input ici pour pouvoir ré-importer le même fichier si besoin
